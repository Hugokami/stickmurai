...
try {
  ...
  for(const id of ['btn-attack','btn-dash','btn-enhance','btn-stance-switch']) if(p.positions?.[id] && Number.isFinite(p.positions[id].x) && Number.isFinite(p.positions[id].y)) qolSettings.positions[id]={x:clamp(p.positions[id].x,.05,.95),y:clamp(p.positions[id].y,.1,.9)};
...
export function requestResume() {
  ...
  const tick=()=>{if(token!==resumeToken||document.hidden||globals.gameState!=='paused')return;if(--count===0){panel.hidden=true;clearGameInputs();document.getElementById('pause-screen')!.style.display='none';AdManager.showMidrollAd(()=>{globals.gameState='playing';AdManager.gameplayStart();});}else{label.textContent=String(count);window.setTimeout(tick,600);}};
  window.setTimeout(tick,600);
}
...
function applySettings() {
  ...
  const ids=['btn-attack','btn-dash','btn-enhance','btn-stance-switch'];
