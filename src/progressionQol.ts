...
export function renderStageBriefing(stage: number, anchor: HTMLElement) {
  ...
  el.textContent=`${objective} · ${ja?'報酬':'Clear reward'}: ${reward.toLocaleString()} 🪙${first?' (×3 first clear)':''} · ★ ${ja?'30回 パリィ&回避':'30 Parries & Dodges'} · ★ ${ja?'残体力 >80%':'>80% HP'} · +300 🪙 ${ja?'初の三つ星':'first 3-star clear'}`;
